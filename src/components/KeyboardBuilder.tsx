...
export default function KeyboardBuilder() {
  ...
  return (
    <section className="py-16 border-t border-gray-200 relative overflow-hidden bg-white text-[#111111]" id="customizer">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          <span className="text-xs text-gray-700 uppercase tracking-widest font-bold border border-gray-200 px-3 py-1 rounded-full bg-gray-50">
            Laboratory
          </span>
          <h2 className="text-[#111111] text-3xl md:text-4xl font-bold font-display mt-4 mb-2">
            Configure Your Centerpiece
          </h2>
          <p className="text-gray-600 text-sm max-w-xl mx-auto font-normal leading-relaxed">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7 flex flex-col items-center justify-center p-8 bg-[#F9F9F9] border border-gray-200 rounded-3xl min-h-[380px] relative shadow-sm">
            {/* Keyboard preview */}
            <motion.div
              animate={{
              ...
              className="w-full max-w-[480px] aspect-[10/3.8] rounded-2xl p-4 border flex flex-col justify-between shadow-lg relative z-10"
            ...
            <span className="text-[10px] text-gray-500 uppercase tracking-widest mt-6 font-bold">
          ...
          <div className="lg:col-span-5 flex flex-col gap-8">
            {/* Switches */}
            <div>
              <span className="text-gray-700 text-[10px] uppercase font-bold tracking-wider mb-3 flex items-center gap-1.5">
              ...
              <div className="grid grid-cols-3 gap-2">
                {switches.map((item) => (
                  ...
                  >
                    ...
                    <span className="block text-[9px] opacity-70 mt-0.5">{item.desc}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {keycaps.map((item) => (
                  ...
                  >
                    <span className="block text-xs font-semibold">{item.name.split(" ")[0]}</span>
                    <span className="block text-[9px] opacity-70 mt-0.5">+{item.price / 1000}k LKR</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="pt-6 border-t border-gray-200 flex items-center justify-between">
              <div>
                <span className="text-[10px] text-gray-500 uppercase tracking-widest block font-bold">Estimated Cost</span>
                <span className="text-[#111111] font-display text-xl font-bold tracking-tight">
                  {new Intl.NumberFormat("en-LK", { style: "currency", currency: "LKR", minimumFractionDigits: 0 }).format(totalPrice)}
                </span>
              </div>
              <button
                onClick={handleAddCustomToCart}
                className="bg-black hover:bg-neutral-800 text-white px-6 py-3 rounded-xl font-semibold text-xs transition-colors shadow-sm cursor-pointer"
